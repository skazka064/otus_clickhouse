## 🎤 **Доклад по проекту «Гибридный ETL-пайплайн для прогнозирования валютных курсов»**

---

### 1. Вступление

Здравствуйте! Тема моей работы — **разработка гибридного ETL-пайплайна для прогнозирования валютных курсов в реальном времени**.

Ключевая проблема финансовых рынков: данные поступают непрерывно, но для принятия решений нужен не только сигнал «здесь и сейчас», но и понимание общего тренда. Поэтому я создал систему, которая объединяет **мгновенный сигнал** и **часовое подтверждение**.

Цель — автоматически выдавать торговые рекомендации (покупать/продавать/ждать) с минимальной задержкой, но с максимальной надёжностью.

---

### 2. Архитектура системы

Архитектура построена на двух параллельных каналах обработки данных: **потоковом** и **пакетном**.
```
Описание архитектуры
    Система построена на четырёх основных компонентах:
        ClickHouse — основное хранилище данных. Здесь созданы две таблицы: ticks для данных из Airflow и ticks_kafka для данных из Kafka. Также в ClickHouse реализовано прогнозирование через материализованные представления.
        Apache Airflow — оркестратор пакетной загрузки. DAG forex_final запускается каждые 15 минут, запрашивает курсы валют из REST API и сохраняет их в таблицу ticks.
        Apache Kafka + Producer — потоковая загрузка. Python-скрипт через WebSocket подключается к Finnhub, получает обновления курсов каждые 1-2 секунды и отправляет их в топик Kafka forex_ticks. ClickHouse автоматически читает этот топик и сохраняет данные в таблицу ticks_kafka.
        Apache Superset — визуализация. На дашборде отображаются графики курсов валют из двух источников, прогноз и метрики точности.
 ```      

```mermaid
graph TD
    subgraph sources["1️⃣ ПОТОКОВЫЙ СИГНАЛ (Kafka)"]
        A[Kafka forex_ticks] --> B[kafka_queue]
        B -->|MV| C[ticks_kafka]
        C -->|MV| E[predictions_eur_usd]
        E -->|VIEW| H[latest_signals]
        H -->|BUY/SELL| I[Superset]
    end

    subgraph dag["2️⃣ ТРЕНДОВОЕ ПОДТВЕРЖДЕНИЕ (Airflow)"]
        DAG[Airflow DAG hourly] -->|INSERT| T[ticks]
        T -->|hourly trend| I
    end

    subgraph decision["🎯 ПРИНЯТИЕ РЕШЕНИЙ"]
        I -->|Анализ: сигнал + тренд| Decision[Торговое решение]
    end
```
2.1. Потоковый канал (Kafka) — «Мгновенный сигнал»

Первый канал — потоковый. Данные с биржи поступают в Kafka каждые 2-3 секунды.

Дальше они автоматически загружаются в ClickHouse через Kafka Engine. В ClickHouse мы строим материализованные представления, которые прямо на лету вычисляют скользящее среднее по последним 10 тикам. Это и есть наш прогноз цены.

Сравнивая прогноз с текущей ценой, система генерирует сигнал:
Условие	Сигнал	Значение
actual_price > predicted_price	BUY	Цена ниже прогноза → ожидаем рост
actual_price < predicted_price	SELL	Цена выше прогноза → ожидаем падение
actual_price ≈ predicted_price	HOLD	Разница минимальна → ничего не делаем

Этот сигнал обновляется каждый новый тик и сразу отображается в Superset.
sql

-- Как вычисляется сигнал в VIEW latest_signals
CASE
    WHEN actual_price > predicted_price THEN 'BUY'
    WHEN actual_price < predicted_price THEN 'SELL'
    ELSE 'HOLD'
END AS signal

2.2. Пакетный канал (Airflow) — «Трендовое подтверждение»

Второй канал — пакетный. Раз в час Airflow загружает срез данных из того же API в отдельную таблицу ticks.

На основе этих часовых данных мы строим график тренда — видим, куда рынок двигался за последние сутки: вверх, вниз или боковик.

Это важно, потому что одиночный сигнал может быть ложным. Например, при сильном восходящем тренде даже случайный сигнал SELL часто оказывается ошибочным.

Решение: мы совершаем сделку только когда сигнал совпадает с трендом:
Сигнал (Kafka)	Тренд (Airflow)	Решение	Обоснование
BUY	UP	✅ ПОКУПКА	Сигнал совпадает с трендом
SELL	DOWN	✅ ПРОДАЖА	Сигнал совпадает с трендом
BUY	DOWN	⚠️ ЖДАТЬ	Противоречие: рынок падает
SELL	UP	⚠️ ЖДАТЬ	Противоречие: рынок растёт
HOLD	ANY	⏸️ НЕ ТОРГОВАТЬ	Нет чёткого сигнала

3. Демонстрация
3.1. Дашборд Superset

На дашборде три зоны:

    Сигналы в реальном времени — сверху мы видим текущую цену и сигнал для каждой валюты.

    Часовой тренд — график цены за последние сутки с отметкой направления.

    Итоговое решение — зелёная галочка, если сигнал подтверждён трендом, и красный крест, если нет.

    ```
-- Запрос для отображения последних сигналов
SELECT * FROM forex_data.latest_signals;
    ```

SELECT 
    timestamp,
    actual_price,
    predicted_price,
    CASE 
        WHEN actual_price > predicted_price THEN 'BUY'
        WHEN actual_price < predicted_price THEN 'SELL'
        ELSE 'HOLD'
    END AS signal
FROM forex_data.predictions_eur_usd
WHERE timestamp > now() - INTERVAL 5 MINUTE
ORDER BY timestamp DESC
LIMIT 10;

4. Технические детали
Компонент	Назначение	Почему выбран
ClickHouse	Хранилище временных рядов	Скорость, сжатие данных в 5-10 раз, мгновенные агрегации
Kafka	Буферизация потоковых данных	Надёжность, масштабируемость, отказоустойчивость
Airflow	Оркестрация пакетной загрузки	Гибкое расписание, мониторинг, логгирование
Superset	Визуализация дашбордов	Интерактивность, поддержка ClickHouse, бесплатный
Materialized Views	Автоматический пересчёт прогнозов	Не нужно запускать отдельные задачи, обновление в реальном времени

5. Результаты и выводы

Что мы получили в итоге:

✅ Гибридный пайплайн — объединили скорость потоковой обработки с надёжностью пакетной.

✅ Автоматические торговые сигналы — BUY/SELL/HOLD с учётом тренда.

✅ Визуализация в реальном времени — дашборд, который помогает принимать решения.

✅ Материализованные представления — прогнозы пересчитываются автоматически при поступлении новых данных.

✅ Двухуровневая логика — сигнал + тренд для фильтрации ложных сигналов.

Ключевые цифры:
Метрика	Значение
Записей в ticks_kafka	288,000+
Прогнозов для EUR/USD	71,000
Прогнозов для GBP/USD	114,000
Прогнозов для USD/JPY	99,000
Задержка обновления сигнала	2-3 секунды
Время выполнения прогноза	< 1 мс
6. Заключение

В перспективе систему можно развивать:

    Добавить больше валютных пар

    Подключить машинное обучение для более сложных прогнозов (LSTM, Prophet)

    Интегрировать отправку сигналов в Telegram или другие мессенджеры

    Настроить автоматическое исполнение сделок через API брокера

    Добавить мониторинг и алертинг (Prometheus + AlertManager)

Спасибо за внимание! Готов ответить на ваши вопросы.



    

### Kafka
Producer — это Python-скрипт, который работает как мост между внешним API и нашей системой.
Он постоянно слушает WebSocket Finnhub и каждое обновление курса отправляет в Kafka. 
Данные передаются в формате CSV, что упрощает их дальнейшую обработку в ClickHouse.
Bid и ask — это две ключевые цены на бирже.
Bid показывает, по какой цене мы можем продать актив, ask — по какой купить. 
Разница между ними называется спредом и является основным источником дохода брокеров.
В скрипте я искусственно создаю bid и ask, чтобы продемонстрировать эту концепцию.
